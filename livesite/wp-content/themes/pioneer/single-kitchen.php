...
<!-- BEGIN MAIN SECTION -->
	<div id="main" >
		...
		<div class="product">
						...
						<?php if(get_field('available_finishes')): ?>
							...
							<?php while(has_sub_field('available_finishes')): ?>
								<li <?php if($filter==$i){ echo "class='active'"; } ?>>
									<figure>
										<a href="#<?php the_sub_field('model'); ?>" title="<?php echo $i;?>" class="open_popup" rel="<?php the_sub_field('model'); ?>"><img src="<?php the_sub_field('color_swatch'); ?>" alt=""></a>
									</figure>
          ...
          <a href="<?php the_field('disclaimer_link'); ?>"><?php if(get_field('part_number')) {?>&raquo; Part</strong> #<?php the_field('part_number'); ?><?php } ?></a>
        </span>
				<div class="share">
      ...
      $post = get_the_ID();
      $cat = get_the_terms( $post, "kitchen_category" );
      $cat = array_shift($cat);
      $cat = $cat->slug;
      $args = array(
        'posts_per_page' => 9,
